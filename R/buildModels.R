...
createThresholdPlot <- function(stats.df, plotFile){

    # Melt the dataframe
    thresh.df <- reshape2::melt(stats.df,
                                id.vars = "threshold",
                                variable.name = "Metric",
                                value.name = "Value") %>%
        dplyr::filter(Metric %in% c("sensitivity", "specificity", "ppv"))
    
    # Create the plot
    thresh.plot <- ggplot2::ggplot(thresh.df) +
        ggplot2::geom_line(ggplot2::aes(x = threshold,
                                        y = Value,
                                        color = Metric)) +
        ggplot2::theme_minimal(base_size = 15)
